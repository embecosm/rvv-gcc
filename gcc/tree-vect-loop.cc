			       ...
			       int reduc_index, vec_loop_masks *masks)
{
  ...
  FOR_EACH_VEC_ELT (vec_oprnds0, i, def0)
	...
	mask = vect_get_loop_mask (gsi, masks, vec_num, vectype_in, i);
      if (LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo))
	len = vect_get_loop_len (gsi, loop_vinfo,
			  ...
			  gimple **vec_stmt, slp_tree slp_node)
{
  ...
  bool length_p = (cond_fn != IFN_LAST
		   && direct_internal_fn_supported_p (length_cond_fn,
						      vectype_in,
						      OPTIMIZE_FOR_SPEED));
			     ...
			     stmt_vector_for_cost *cost_vec)
{
  ...
  if (loop_vinfo)
      ...
      else if (LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo))
	{
	  ...
	  gcc_assert (ncopies == 1 && !slp_node);
	  tree scalar_type = TREE_TYPE (STMT_VINFO_VECTYPE (stmt_info));
	  tree len = vect_get_loop_len (gsi, loop_vinfo,
					&LOOP_VINFO_LENS (loop_vinfo), 1,
					vectype, 0);
	  tree scalar_res = gimple_build (&stmts, CFN_EXTRACT_LAST, scalar_type,
					  len, vec_lhs_phi);
  ...
  if (maybe_ne (TYPE_VECTOR_SUBPARTS (rgl->type),
		TYPE_VECTOR_SUBPARTS (vectype)))
    {
	 ...
	 are N times bigger than X's.  */
      ...
      gimple_seq seq = NULL;
      len = gimple_build (&seq, RSHIFT_EXPR, len_type, len,
			  build_int_cst (len_type, exact_log2 (factor)));
      if (seq)
	gsi_insert_seq_before (gsi, seq, GSI_SAME_STMT);
    }
