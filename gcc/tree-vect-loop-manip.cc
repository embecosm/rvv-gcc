...
static tree
vect_set_loop_controls_for_while_len (class loop *loop,
				      ...
				      tree, bool)
{
  ...
  FOR_EACH_VEC_ELT_REVERSE (rgc->controls, i, ctrl)
	...
	{
	  init_ctrl
	    = gimple_build (preheader_seq, IFN_WHILE_LEN, compare_type,
			    this_test_limit,
				 ...
				 tree niters_skip, bool might_wrap_p)
{
  ...
  if (!use_masks_p && !niters_skip
      && direct_internal_fn_supported_p (IFN_WHILE_LEN, compare_type, iv_type,
					 OPTIMIZE_FOR_SPEED))
    return
      vect_set_loop_controls_for_while_len (loop, loop_vinfo, preheader_seq,
					    header_seq, loop_cond_gsi, rgc,
					    niters, niters_skip, might_wrap_p);
