      ...
      FOR_EACH_VEC_ELT (vec_oprnds0, i, vop0)
	{
	  ...
	  vec_loop_lens *lens
	    = (loop_vinfo && LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo)
	       ...
	       : NULL);
	  internal_fn len_fn
	    = get_length_conversion_internal_fn (code1, TREE_TYPE (vec_dest),
						 TREE_TYPE (vop0));
		    ...
		    stmt_vector_for_cost *cost_vec)
{
  ...
  FOR_EACH_VEC_ELT (vec_oprnds0, i, vop0)
	...
	= loop_vinfo && LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo);
      internal_fn len_fn
	= get_with_length_shift_internal_fn (code,
  ...
  FOR_EACH_VEC_ELT (vec_oprnds0, i, vop0)
      ...
      else
	{
	  ...
	  /* FIXME: For fully control with length vector and unconditional
	     ...
	     operations so that they explicitly ignore the inactive elements.
	     In RISC-V 'V' Extension, we prefer agnostic value for tailed
	     elements meaning we don't care about the value for tailed
	     elements in most cases, so don't need to bring the 'DEST' into
	     the argument.  But we are not sure about other targets like MVE
	     or PowerPC.  */
	  ...
	  if (bitop1 == NOP_EXPR)
	    {
		  ...
		  && (get_len_vec_cmp_icode
		      (TYPE_MODE (TREE_TYPE (vec_cond_lhs)),
		       TYPE_MODE (vec_cmp_type), unsigned_p)
		      != CODE_FOR_nothing))
		{
		...
		{
		  if (len
		      && direct_internal_fn_supported_p (IFN_LEN_NOT,
							 vec_cmp_type,
							 OPTIMIZE_FOR_SPEED))
		    {
	      ...
	      else
		{
		  ...
		  if (len && len_fn != IFN_LAST
		    ...
		    {
		      new_stmt
			= gimple_build_call_internal (len_fn, 3, vec_cond_lhs,
						      vec_cond_rhs, len);
		      gimple_call_set_lhs (new_stmt, new_temp);
		    }
		  else
      ...
      if (bitop1 == NOP_EXPR)
	{
	  ...
	  if (len
		   ...
		   unsignedp) != CODE_FOR_nothing))
	    {
	      ...
	      new_stmt
		= (gimple_build_call_internal
		   (unsignedp ? IFN_LEN_VEC_CMPU : IFN_LEN_VEC_CMP, 4,
		    vec_rhs1, vec_rhs2, len, code_tree));
	      gimple_call_set_lhs (new_stmt, new_temp);
	    }
	  else
      ...
      else
	{
	  if (bitop1 == BIT_NOT_EXPR)
	  ...
	  else
	    {
	      internal_fn len_fn = get_with_length_internal_fn (bitop1);
	      if (len_fn != IFN_LAST
		  && direct_internal_fn_supported_p (len_fn,
						     TREE_TYPE (new_temp),
						     OPTIMIZE_FOR_SPEED))
		{
	      ...
	      else
		{
		  ...
		  if (len_fn != IFN_LAST
		      && direct_internal_fn_supported_p (len_fn,
							 ...
							 OPTIMIZE_FOR_SPEED))
		    {
		      new_stmt = gimple_build_call_internal (len_fn, 3,
							     vec_rhs1,
							     new_temp, len);
		      gimple_call_set_lhs (new_stmt, res);
		    }
			       ...
			       tree *var_before, tree *var_after)
{
  ...
  *var_after = va;
  ...
  tree var
    = gimple_build (header_seq, IFN_WHILE_LEN, compare_type, ctrl,
