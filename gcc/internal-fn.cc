...
#define DEF_INTERNAL_SIGNED_CONVERT_OPTAB_FN(CODE, FLAGS, SELECTOR1,	\
					     SELECTOR2, SIGNED_OPTAB,	\
					     UNSIGNED_OPTAB,		\
					     TYPE) TYPE##_direct,
#include "internal-fn.def"
  not_direct
...
static void
expand_vec_convert_binary_v_optab_fn (internal_fn, gcall *stmt,
				      convert_optab optab)
{
  int i = 0;
  class expand_operand ops[4];
  ...
  if (optab == len_vec_convert_vsaddv_optab
      || optab == len_vec_convert_vuaddv_optab
      || optab == len_vec_convert_saddv_optab
      || optab == len_vec_convert_uaddv_optab
      || optab == len_vec_convert_vssubv_optab
      || optab == len_vec_convert_vusubv_optab
      || optab == len_vec_convert_ssubv_optab
      || optab == len_vec_convert_usubv_optab
      || optab == len_vec_convert_vsmulv_optab
      || optab == len_vec_convert_vumulv_optab
      || optab == len_vec_convert_vsumulv_optab
      || optab == len_vec_convert_smulv_optab
      || optab == len_vec_convert_umulv_optab
...
static void
expand_vec_convert_binary_w_optab_fn (internal_fn, gcall *stmt,
				      convert_optab optab)
{
  int i = 0;
  class expand_operand ops[4];
  ...
  if (optab == len_vec_convert_vsaddw_optab
      || optab == len_vec_convert_vuaddw_optab
      || optab == len_vec_convert_saddw_optab
      || optab == len_vec_convert_uaddw_optab
      || optab == len_vec_convert_vssubw_optab
      || optab == len_vec_convert_vusubw_optab
      || optab == len_vec_convert_ssubw_optab
      || optab == len_vec_convert_usubw_optab)
    {
      tree len = gimple_call_arg (stmt, 2);
      rtx len_rtx = expand_normal (len);
      create_convert_operand_from (&ops[i++], len_rtx,
...
static void
expand_vec_convert_ternary_optab_fn (internal_fn, gcall *stmt,
				     convert_optab optab)
{
  int i = 0;
  class expand_operand ops[5];
...
expand_scatter_store_optab_fn (internal_fn, gcall *stmt, direct_optab optab)
{
  ...
  if (mask_index >= 0)
	...
	create_convert_operand_from (&ops[i++], mask_rtx,
...
#define DEF_INTERNAL_SIGNED_CONVERT_OPTAB_FN(CODE, FLAGS, SELECTOR1,	\
	...
	optab which_optab = (TYPE_UNSIGNED (types.SELECTOR1)		\
			     ? UNSIGNED_OPTAB ## _optab			\
			     : SIGNED_OPTAB ## _optab);			\
	return convert_optab_handler (which_optab,			\
				      TYPE_MODE (types.SELECTOR1),	\
				      TYPE_MODE (types.SELECTOR2));	\
      }
#include "internal-fn.def"
...
/* Invoke T(CODE, IFN) for each length control function IFN that maps to a
   tree code CODE.  */
#define FOR_EACH_CODE_MAPPING_WITH_LENGTH(T) \
...
/* Invoke T(IFN, IFN) for each length control function IFN that maps to a
   tree code CODE.  */
#define FOR_EACH_IFN_MAPPING_WITH_LENGTH(T) \
...
get_with_length_internal_fn (tree_code code)
{
  switch (code)
    {
#define CASE(CODE, IFN) case CODE: return IFN;
      FOR_EACH_CODE_MAPPING_WITH_LENGTH(CASE)
#undef CASE
    default:
      return IFN_LAST;
    }
...
get_with_length_internal_fn (internal_fn ifn)
{
  switch (ifn)
    {
#define CASE(CODE, IFN) case CODE: return IFN;
      FOR_EACH_IFN_MAPPING_WITH_LENGTH(CASE)
#undef CASE
    default:
      return IFN_LAST;
    }
...
/* Invoke T(CODE, IFN) for each length control function IFN that maps to a
   tree code CODE.  */
#define FOR_EACH_CODE_MAPPING_WITH_LENGTH_TAIL(T) \
...
get_with_length_tail_internal_fn (tree_code code)
{
  switch (code)
    {
#define CASE(CODE, IFN) case CODE: return IFN;
      FOR_EACH_CODE_MAPPING_WITH_LENGTH_TAIL (CASE)
#undef CASE
    default:
      return IFN_LAST;
    }
