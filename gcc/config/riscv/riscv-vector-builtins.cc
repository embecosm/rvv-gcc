...
#define DEFINE_SCALAR_PTR_TYPE_NODE(NBITS)				\
  ...
  const_scalar_pointer_types[VECTOR_TYPE_int##NBITS]			\
    = build_pointer_type (build_type_variant				\
			  (scalar_types[VECTOR_TYPE_int##NBITS],	\
			   1, 0));					\
  const_scalar_pointer_types[VECTOR_TYPE_uint##NBITS]			\
    = build_pointer_type (build_type_variant				\
			  (scalar_types[VECTOR_TYPE_uint##NBITS],	\
			   1, 0));
  ...
  const_scalar_pointer_types[VECTOR_TYPE_float16]
    = (build_pointer_type
       (build_type_variant (scalar_types[VECTOR_TYPE_float16], 1, 0)));
  const_scalar_pointer_types[VECTOR_TYPE_float32]
    = (build_pointer_type
       (build_type_variant (scalar_types[VECTOR_TYPE_float32], 1, 0)));
  const_scalar_pointer_types[VECTOR_TYPE_float64]
    = (build_pointer_type
			     ...
			     riscv_vector::vector_arg_attr_info attr, ...)
{
  ...
  patterns.arg_list
    = ((riscv_vector::vector_mode_attr_list **)
       xmalloc (len * sizeof (riscv_vector::vector_mode_attr_list *)));
...
init_def_variables ()
{
  ...
  uint64_t pred_reduce = (PRED_void | PRED_ta | PRED_tu | PRED_m | PRED_tam
			  | PRED_tum);
  ...
  uint64_t pat_void_dest_ignore_mp = (PAT_mask | PAT_tail | PAT_void_dest
				      | PAT_ignore_mask_policy);
...
#define DEF_RISCV_ARG_MODE_ATTR_VARIABLE(VARIABLE_NAME, ELEM_CNT)	\
  VVAR (VARIABLE_NAME)							\
    = {ELEM_CNT,							\
       ((riscv_vector::vector_mode_attr *)				\
	xmalloc (ELEM_CNT * sizeof (vector_mode_attr)))};
...
#undef DEF_RISCV_ARG_MODE_ATTR
  ...
  NUM_INSN_FUNC = 0;
#define DEF_RVV_FUNCTION(BASE_NAME, CLASS_NAME, ARG_PATTERN,		\
			 INTRINSIC_PATTERN, PREDS, OP_TYPES)		\
  NUM_INSN_FUNC++;
#include "riscv-vector-builtins-functions.def"
#undef DEF_RVV_FUNCTION

  all_vector_functions
    = ((riscv_vector::function_builder **)
       xmalloc (sizeof (riscv_vector::function_builder *) * NUM_INSN_FUNC));
...
#define DEF_RVV_FUNCTION(BASE_NAME, CLASS_NAME, ARG_PATTERN,		\
  ...
  all_vector_functions[func_idx++]					\
    = (new riscv_vector::CLASS_NAME					\
       (#BASE_NAME, get_vector_arg_all_patterns ARG_PATTERN,		\
	INTRINSIC_PATTERN, PREDS, OP_TYPES, (REQUIRED_EXTENSIONS)));
#include "riscv-vector-builtins-functions.def"
#undef DEF_RVV_FUNCTION
}
