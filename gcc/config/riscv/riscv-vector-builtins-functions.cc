		      ...
		      bool reverse_p = false)
{
  ...
  const char *dt1 = mode2data_type_str (TYPE_MODE (vectype), unsigned_p,
					false);
  const char *pred = get_pred_str (instance.get_pred ());
  ...
  registered_function *rfn_slot = function_table->find_with_hash (fn_instance,
								  hashval);
		   ...
		   rtx target) const
{
  ...
  if (target && GET_MODE_NUNITS (mode).is_constant ())
    {
      riscv_emit_move (target,
		       GEN_INT (GET_MODE_NUNITS (mode).to_constant ()));
      return target;
    }
...
vset::expand (const function_instance &, tree exp, rtx target) const
{
  ...
  unsigned int nvecs
    = exact_div (GET_MODE_SIZE (GET_MODE (target)),
  ...
  rtx subreg = simplify_gen_subreg (GET_MODE (op2), target, GET_MODE (target),
				    offset);
...
vget::expand (const function_instance &, tree exp, rtx target) const
{
  ...
  rtx op1 = expand_normal (arg1);
  unsigned int nvecs
    = exact_div (GET_MODE_SIZE (GET_MODE (op0)),
