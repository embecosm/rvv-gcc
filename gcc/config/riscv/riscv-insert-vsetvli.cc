...
public:
  ...
  // -----+--------------+------------------------------------------------
  // 31   | vma_mutate_p | Vector mask agnostic
  // 30   | vta_mutate_p | Vector tail agnostic
...
needvsetvli (rtx_insn *insn, const vinfo &require, const vinfo &curr_info)
{
  ...
  // We didn't find a compatible value.  It might be defined by a
  // VSET(I)VLI.  If it has the same VTYPE we need and the last VL/VTYPE
  // we observed is the same, we don't need a VSETVLI here.
  ...
  if (fault_first_load_p (insn))
    {
      ...
      for (rtx_insn *next_insn = NEXT_INSN (insn);
	   next_insn && next_insn != NEXT_INSN (BB_END (bb));
      ...
      if (reload_completed)
	{
	  ...
	  if (prev_info.valid_p () && !prev_info.unknown_p ()
	  ...
	  else
	    {
	      ...
	      PUT_MODE (avl, Pmode);
	      gcc_assert (HARD_REGISTER_P (avl)
			  && !rtx_equal_p (avl,
					   gen_rtx_REG (Pmode, X0_REGNUM)));
	    }
	}
