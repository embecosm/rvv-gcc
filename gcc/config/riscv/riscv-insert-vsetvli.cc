...
public:
  ...
  load_store_compatible_p (unsigned vsew_arg, const vinfo &info) const
  {
    ...
    return calc_sew_lmul_ratio () == ::calc_sew_lmul_ratio (vsew_arg,
							    info.vlmul);
  }
...
needvsetvli (rtx_insn *insn, const vinfo &require, const vinfo &curr_info)
{
  ...
  return (curr_info.unknown_p ()
	  || !can_skip_vsetvli_for_load_store_p (insn, require, curr_info));
}
	...
	if (use_vl_p (insn))
	  {
	    ...
	    // 1) If the AVL is the result of a previous vsetvli which has
	    //    the same AVL and VLMAX as our current state, we can reuse
	    //    the AVL from the current state for the new one.  This
	    //    allows us to generate 'vsetvli x0, x0, vtype" or possibly
			...
			if (!curr_info.unknown_p ()
			    ...
			    && def_info.vlmax_equal_p (curr_info))
			  {
			    replace_op (insn, curr_info.get_avl (),
					REPLACE_VL);
			    curr_info = compute_info_for_instr (insn,
								curr_info);
			    continue;
			  }
			 ...
			 const demanded_fields &used)
{
  ...
  if (used.tail_policy
     && (riscv_parse_vta_field (INTVAL (vtype))
	 != riscv_parse_vta_field (INTVAL (prior_vtype))))
    return false;
  if (used.mask_policy
     && (riscv_parse_vma_field (INTVAL (vtype))
	 != riscv_parse_vma_field (INTVAL (prior_vtype))))
    return false;
  return true;
}
