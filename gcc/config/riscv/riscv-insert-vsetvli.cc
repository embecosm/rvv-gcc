  ...
  if (recog_data.n_operands == 1)
      ...
      else
	{
	  ...
	  for (next_insn = NEXT_INSN (insn); insn != NEXT_INSN (BB_END (bb));
		...
		{
		  vinfo next_info = compute_info_for_instr (next_insn,
							    curr_info);
		  ...
		  if ((recog_clobber_vl_vtype (next_insn)
		       != MOV_CLOBBER_REG_REG)
      ...
      if (prev_insn)
	{
	  if (!used.vl && !used.used_vtype ())
	  ...
	  else if (!used.used_vtype () && vl_preserving_config_p (insn))
	    {
	      ...
	      // for this case.  If you find yourself wanting to add other
	      // forms to this "unused VTYPE" case, we're probably missing
	      // a canonicalization earlier.
