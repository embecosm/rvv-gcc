...
riscv_vector_vrgather (struct expand_vec_perm_d *d)
{
  ...
  if (d->perm.series_p (0, 1, d->perm.length () - 1, -1))
	...
	{
	  emit_insn (gen_vmul_vx_32bit (sel_mode, sel_reg, const0_rtx,
					const0_rtx, sel_reg,
					force_reg (Pmode, GEN_INT (-1)),
					gen_rtx_REG (Pmode, X0_REGNUM),
					riscv_vector_gen_policy ()));
	  emit_insn (gen_vadd_vx_32bit (sel_mode, sel_reg, const0_rtx,
	...
	{
	  emit_insn (gen_v_vx (UNSPEC_VMUL, sel_mode, sel_reg, const0_rtx,
			       const0_rtx, sel_reg, GEN_INT (-1),
			       gen_rtx_REG (Pmode, X0_REGNUM),
			       riscv_vector_gen_policy ()));
	  emit_insn (gen_v_vx (UNSPEC_VADD, sel_mode, sel_reg, const0_rtx,
			       const0_rtx, sel_reg, base,
			       gen_rtx_REG (Pmode, X0_REGNUM),
			       riscv_vector_gen_policy ()));
	}
