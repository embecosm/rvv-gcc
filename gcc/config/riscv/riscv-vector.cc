    ...
    int ending_others, int n_mismatch, int base, int step)
{
  ...
  for (int i = 0; i < nelts_reqd; i++)
	...
	x = GEN_INT (base + step * (i - ending_others));
      else if (i >= ending_others)
	x = builder.elt (i - ending_others);
      else
	x = CONST0_RTX (elem_mode);
      v.quick_push (x);
    }
...
static bool
riscv_vector_expand_strided_init (rtx target,
				  ...
				  int nelts_reqd)
{
  ...
  for (int i = 0; i < 2; i++)
    {
      ...
      if (!(is_a<gassign *> (SSA_NAME_DEF_STMT
			     (TREE_OPERAND (vars[start], 0)))))
	break;
      gassign *assign
      ...
      for (int j = start + 1; j < end; j++)
	{
	  ...
	  if (!(is_a<gassign *> (SSA_NAME_DEF_STMT
				 (gimple_assign_rhs1 (assign2)))))
	    {
	      all_same_stride_p = false;
	      break;
	    }
	  gassign *assign3 = as_a<gassign *> (SSA_NAME_DEF_STMT
					      (gimple_assign_rhs1 (assign2)));
  ...
  if (stride[0] && stride[1])
	...
	mode, force_reg (Pmode, XEXP (builder.elt (nelts_reqd / 2), 0)));
      ...
      emit_insn (gen_vslide_vx (UNSPEC_SLIDEUP, mode, target, const0_rtx,
				const0_rtx, reg,
				gen_int_mode (nelts_reqd / 2, Pmode),
				gen_rtx_REG (Pmode, X0_REGNUM),
				riscv_vector::gen_tu_policy (),
...
static bool
riscv_vector_expand_indexed_init (rtx target,
				  const rtx_vector_builder &builder,
				  int nelts_reqd)
{
  if (nelts_reqd < 4)
...
riscv_vector_expand_vec_perm (rtx target, rtx op0, rtx op1, rtx sel)
{
  ...
  if (CONST_VECTOR_P (sel)
	...
	emit_insn (gen_vrgather_vv (data_mode, target, const0_rtx,
				    const0_rtx, op0, sel_reg,
				    gen_rtx_REG (Pmode, X0_REGNUM),
				    riscv_vector_gen_policy (),
				    gen_reg_rtx (Pmode)));
      return;
...
riscv_vector_vrgather (struct expand_vec_perm_d *d)
{
  ...
  if (d->perm.series_p (0, 1, d->perm.length () - 1, -1))
	...
	= (GET_MODE_BITSIZE (inner_mode) == 64 && !TARGET_64BIT
	   ? Pmode
	   : inner_mode);
...
riscv_vector_expand_const_vector (rtx target, rtx src)
{
  ...
  if (!GET_MODE_SIZE (mode).is_constant ())
	...
	gcc_assert (riscv_vector_data_mode
  ...
  if (CONST_VECTOR_DUPLICATE_P (src))
      ...
      for (unsigned int i = 1; i < npatterns; i++)
	{
	  ...
	  if (FLOAT_MODE_P (mode))
	    ...
	    emit_insn (gen_v_vxm (UNSPEC_VMERGE, mode, target, mask,
				  const0_rtx, target, base,
				  gen_rtx_REG (Pmode, X0_REGNUM),
				  riscv_vector_gen_policy (),
				  gen_reg_rtx (Pmode)));
	}
	  ...
	  if (nbits == 64 && !TARGET_64BIT && CONST_POLY_INT_P (base1))
	    {
	      ...
	      emit_insn (gen_vadd_vx_32bit
			 (mode, target, gen_lowpart (mask_mode, pseudo_mask),
			  target, target, reg, gen_rtx_REG (Pmode, X0_REGNUM),
			  riscv_vector_gen_policy (), gen_reg_rtx (Pmode)));
	    }
	  else
	    emit_insn (gen_v_vx (UNSPEC_VADD, mode, target,
