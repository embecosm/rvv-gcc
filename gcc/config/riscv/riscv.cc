	  ...
	  if (riscv_tuple_mode_p (mode))
	    {
	      ...
	      poly_int64 poly_offset (GET_MODE_SIZE (mode).coeffs[0] / nf,
				      GET_MODE_SIZE (mode).coeffs[1] / nf);
	      nvecs = (known_le (poly_offset, BYTES_PER_RISCV_VECTOR)
		       ? 1
		       : exact_div (poly_offset,
				    BYTES_PER_RISCV_VECTOR).to_constant ());
	      nvecs = nvecs * nf;
	    }
	  else
	    nvecs = (known_le (GET_MODE_SIZE (mode), BYTES_PER_RISCV_VECTOR)
		     ? 1
...
riscv_first_stack_step (struct riscv_frame_info *frame)
{
  ...
  HOST_WIDE_INT min_first_step
    = (RISCV_STACK_ALIGN
       ((frame->total_size - frame->fp_sp_offset).to_constant ()));
...
riscv_expand_prologue (void)
{
  ...
  if (known_gt (size, 0))
    {
      /* Two step adjustment, first for scalar frame, second for vector
	 frame.  */
...
riscv_expand_epilogue (int style)
{
  ...
  if ((frame->mask | frame->fmask) != 0)
    riscv_for_each_saved_reg (restore_offset, riscv_restore_reg, true,
			  ...
			  reg_class_t from, reg_class_t to)
{
  /* There is no easy way to move fp16 value between FPR without ZFH
     extension, it requires an extra temp GPR to move that.  */
  if (!TARGET_FP16 && mode == HFmode && from == FP_REGS && to == FP_REGS)
    return 2 * tune_param->fmv_cost;
...
riscv_hard_regno_nregs (unsigned int regno, machine_mode mode)
{
  ...
  if (FP_REG_P (regno))
    return ((GET_MODE_SIZE (mode).to_constant () + UNITS_PER_FP_REG - 1)
	    / UNITS_PER_FP_REG);
...
riscv_modes_tieable_p (machine_mode mode1, machine_mode mode2)
{
  if (riscv_vector_mode_p (mode1) && riscv_vector_mode_p (mode2))
    ...
    {
      /* If only one is vector mode, then don't allow scalar
	 and vector mode to be tied.  */
      return false;
    }
...
expand_crc_lookup (rtx *operands, machine_mode data_mode)
{
  ...
  gcc_assert (GET_MODE_SIZE (data_mode).to_constant ()
  ...
  if (CONST_INT_P (operands[1]) && CONST_INT_P (operands[2]))
	...
	= ((INTVAL (operands[1]) & GET_MODE_MASK (crc_mode))
      ...
      crc_i = compute_crc (crc_i, GET_MODE_BITSIZE (data_mode).to_constant (),
			   polynom);
      emit_insn (gen_rtx_SET (operands[0], GEN_INT (crc_i)));
      return;
    }
  machine_mode mode = crc_mode;
  if (GET_MODE_SIZE (mode).to_constant () < GET_MODE_SIZE (word_mode))
    mode = word_mode;
  gcc_assert (GET_MODE_SIZE (mode).to_constant () == GET_MODE_SIZE (Pmode));
  ...
  rtx tab = print_crc_table (GET_MODE_BITSIZE (crc_mode).to_constant (),
			     GET_MODE_BITSIZE (data_mode).to_constant (),
			     polynom);
				  ...
				  tree vectype, int misalign ATTRIBUTE_UNUSED)
{
  ...
  switch (type_of_cost)
    ...
    case scalar_load:
      return
	vector_tune_param->rvv_stmt_costs_table->scalar_load->cost (x, mode);

    case scalar_store:
      return
	vector_tune_param->rvv_stmt_costs_table->scalar_store->cost (x, mode);

    case vector_stmt:
      return
	(fp
	 ? vector_tune_param->rvv_stmt_costs_table->vec_fp->cost (x, mode)
    ...
    case vector_store:
      return
	vector_tune_param->rvv_stmt_costs_table->vec_store->cost (x, mode);

    case vec_to_scalar:
      return
	vector_tune_param->rvv_stmt_costs_table->vec_to_scalar->cost (x, mode);

    case scalar_to_vec:
      return
	vector_tune_param->rvv_stmt_costs_table->scalar_to_vec->cost (x, mode);
    ...
    case cond_branch_not_taken:
      return
	vector_tune_param->rvv_stmt_costs_table->cond_not_taken_branch->cost
	  (x, mode);

    case vec_perm:
      return
	vector_tune_param->rvv_stmt_costs_table->vec_permute->cost (x, mode);
...
/* Assume for now that it isn't worth branching around empty masked ops
   (including masked stores).  */
