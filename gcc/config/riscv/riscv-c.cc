  ...
  if (strcmp (name, "vector") == 0)
    {
      if (!TARGET_VECTOR)
	error
	  ("%<#pragma riscv intrinsic%> option %qs needs 'V' extension enabled",
	   name);
...
/* Hook to validate the current #pragma riscv intrinsic target
   and set the state, and update the macros based on what was changed.  */
