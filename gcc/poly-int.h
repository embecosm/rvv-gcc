...
struct poly_coeff_pair_traits
{
  ...
  /* True if T1 needs a change to a wider type to prevent a -Wsign-compare
     warning e.g. if T1 is unsigned int, T2 is int, then T1 should be widened
     to HOST_WIDE_INT.  */
  static const bool t1_need_wider_p
    = (poly_coeff_traits<T1>::signedness == 0
	  ...
	  if (q == 0)
	      ...
	      if (a.coeffs[i] != ICa (0))
		{
		  /* Use negative absolute to avoid overflow, i.e.
		     ...
		     then -coeff[i] == ~0x1 + 0x1 == 0xFFFFFFFF with
		     unsigned int type then cast to C,
		     if C is HOST_WIDE_INT type (64bit), then the cast
		     result of coeff[i] is 0x00000000 FFFFFFFF,
		     a positive value, it is not expected.  */
		  C neg_abs_a = (a.coeffs[i] < 0
				 ? a.coeffs[i]
				 : -(C) a.coeffs[i]);
		  C neg_abs_b = (b.coeffs[i] < 0
				 ? b.coeffs[i]
				 : -(C) b.coeffs[i]);
