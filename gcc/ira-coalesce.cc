	...
	for (int i = 0; i < recog_data.n_operands; i++)
	  {
		...
		if (SUBREG_P (recog_data.operand[i])
		    ...
		    && !(paradoxical_subreg_p
			 (GET_MODE (dest),
			  GET_MODE (SUBREG_REG (recog_data.operand[i])))))
		  return false;
	      }
	  }
      }
      ...
      if (INSN_P (insn) && PATTERN (insn))
	{
	  ...
	  for (int i = 0; i < recog_data.n_operands; i++)
		  ...
		  && rtx_equal_p (recog_data.operand[i], dest))
		{
		  ...
		  && rtx_equal_p (SUBREG_REG (recog_data.operand[i]), dest))
		{
		  if (reg_equiv_init (REGNO (SUBREG_REG
					     (recog_data.operand[i])))
		      && ((reg_equiv_init
			   (REGNO (SUBREG_REG
				   (recog_data.operand[i])))->insn ())
		  ...
		  rtx pat
			...
			(simplify_gen_subreg
			 (GET_MODE (recog_data.operand[i]), src,
			  GET_MODE (SUBREG_REG (recog_data.operand[i])),
...
legitimate_subreg_to_subreg_copy_p (rtx_insn *copy)
{
      ...
      if (find_regno_note (insn, REG_DEAD, REGNO (src)))
	{
	  ...
	  for (prev_insn = PREV_INSN (insn);
		  ...
		  && rtx_equal_p (XEXP (PATTERN (prev_insn), 0), dest))
		{
		  ...
		  && rtx_equal_p (SET_SRC (single_set (prev_insn)),
					   const0_rtx))
		{
		  last_insn = insn;
		  break;
  ...
  if (last_insn)
	...
	= (exact_div
	   (offset,
	    GET_MODE_SIZE (GET_MODE (SET_DEST
				     (single_set (copy))))).to_constant ());
      ...
      if (INSN_P (insn) && single_set (insn))
	{
	  ...
	  if (SUBREG_P (src) && SUBREG_P (dest) && REG_P (SUBREG_REG (src))
	      ...
	      && GET_MODE (dest) == GET_MODE (src))
	    {
	      ...
	      for (unsigned i = 0; i < visitedlist.length (); i++)
		if ((rtx_equal_p
		     (SUBREG_REG (SET_SRC (single_set (insn))),
		      SUBREG_REG (SET_SRC (single_set (visitedlist[i])))))
		    && (rtx_equal_p
			(SUBREG_REG (SET_DEST (single_set (insn))),
			 SUBREG_REG (SET_DEST (single_set (visitedlist[i]))))))
		  {
		    visited_p = true;
		    break;
		  }
