...
/* If target supports vector load/store with length for vector mode MODE,
   ...
   There are two flavors for vector load/store with length, one is to measure
   length with bytes, the other is to measure length with lanes.
   As len_{gather,scatter}_{load,store} optabs point out, for the flavor with
   bytes, we use VnQI to wrap the other supportable same size vector modes.  */

opt_machine_mode
get_len_gather_scatter_mode (machine_mode to_mode, machine_mode from_mode,
			     bool is_load)
{
